...
def function(s1, s2):
    ...
    matrix[0, :] = [gap for j in range(cols)]
    matrix[:, 0] = [gap for i in range(rows)]
    matrix[0, 0] = 0
    ...
    for row in range(1, rows):
        for col in range(1, cols):
            score = blosum62[f"{s1[row - 1]}{s2[col - 1]}"]

            matrix[row][col] = score + max(matrix[row - 1][col-1],
                                           Gs1[row - 1][col-1],
                                           Gs2[row - 1][col-1])
            Gs2[row][col] = max(matrix[row-1][col] + gap,
                                Gs2[row-1][col],
                                Gs1[row-1][col] + gap)
            Gs1[row][col] = max(matrix[row][col-1] + gap,
                                Gs1[row][col-1],
                                Gs2[row][col-1] + gap)

    return max(matrix[-1,-1], Gs1[-1,-1], Gs2[-1,-1])
